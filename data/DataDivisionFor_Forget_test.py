...
def prepare_mnist_data():
    ...
    mnist = fetch_openml('MNIST original', data_home='./mnist/data')
...
class DataDivider():
    def __init__(self,data_list,label_list,num_users=50,a=100,division_ratio=[1/3,1/3,1/3],train_test_ratio=0.8,savepath = './cifar10/',num_class=10):
        ...
        self.train_data_remain, self.test_data_remain = self.generate_train_test(self.dataset_remain,0.5)
    ...
    def dist1(self):#distribution 1
        ...
        for u in range(st_idx, end_idx):
            ...
            for i in range(c_st, c_end):
                classes.append(i % (self.num_class-self.num_class//2)) #从前5类中取
            print('@line 289 classes gotten from first half:', classes)
            c_pos += n
        ...
        print('line 292 sample num dist1:', sample_num)
        print('@line293 num of each class in dist1:', c)
        return dataset
    ...
    def dist_last5(self):
        ...
        for u in range(st_idx, end_idx):
            ...
            for i in range(c_st, c_end):
                classes.append(i % (self.num_class-self.num_class//2)+self.num_class//2) #从前5类中取
            print('@line classes gotten from last5:', classes)
            c_pos += n
        ...
        for d_u in dataset.values():
            ...
            ls += d_u['y'].tolist()
        c = collections.Counter(ls)
        print('line 292 sample num dist_last5:', sample_num)
        print('@line293 num of each class in dist_last5:', c)
        return dataset
...
def genrate_cifar10(pretrain=False,user_num=50,a=10):
    ...
    generator=DataDivider(data_list,label_list,num_users=user_num,a=a,division_ratio=[1, 0, 0, 0, 0,0],train_test_ratio=0.5,savepath='/root/TC174611125/fmaml/fmaml_mac/data/cifar10',num_class=10)
    generator.save_data(pretrain=pretrain)

def genrate_cifar100(pretrain=False,user_num=50,a=10):
    print('Generating Cifar100......')
    data_list, label_list=prepare_cifar100()
    generator=DataDivider(data_list,label_list,num_users=user_num,a=a,division_ratio=[1, 0, 0, 0,0, 0],train_test_ratio=0.5,savepath='/root/TC174611125/fmaml/fmaml_mac/data/cifar100',num_class=100)
    generator.save_data(pretrain)
...
def generate_Fmnist(pretrain=True, user_num=50, a=20):
    data_list, label_list=prepare_Fmnist_data()
    generator = DataDivider(data_list, label_list, num_users=user_num, a=a, division_ratio=[1, 0, 0, 0, 0, 0],
                            train_test_ratio=0.5, savepath='/root/TC174611125/fmaml/fmaml_mac/data/Fmnist', num_class=10)
    generator.save_data(pretrain)
...
if __name__=='__main__':
    ...
    if parsed['dataset']=='cifar10':
        ...
        genrate_cifar10(pretrain=True, user_num=50, a=15)
    elif parsed['dataset']=='cifar100':
        print('Preparing cifar100')
        genrate_cifar100(pretrain=True, user_num=50, a=20)
    else:
        print('Preparing Fmnist')
        generate_Fmnist(pretrain=True, user_num=50, a=20)
