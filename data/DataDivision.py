...
class DataDivider():
    ...
    def dist1(self):#distribution 1
        ...
        for u in range(st_idx,end_idx):
            data_u=[]
            label_u=[]
            for class_idx in range(self.num_class//2):
                assert self.idx[class_idx]+self.a<self.data_list[class_idx].shape[0],'in dist1类别%d样本量不足'%class_idx
                data_u.append(self.data_list[class_idx][self.idx[class_idx]:self.idx[class_idx]+self.a])
                label_u.append(self.label_list[class_idx][self.idx[class_idx]:self.idx[class_idx]+self.a])
                self.idx[class_idx]+=self.a
                sample_num+=self.a
            data_u=np.concatenate(data_u)
            label_u=np.concatenate(label_u)
            dataset1[u]={'X':data_u,'y':label_u}
        print('line 124 sample num dist1:', sample_num)
        return dataset1
    ...
    def dist2(self): #ditribution2
        ...
        for u in range(st_idx, end_idx):
            ...
            sample_num+=self.a*2
        print('line 150 sample num dist2:', sample_num)
        return dataset2
    ...
    def dist3(self):
        ...
        print('line 172 sample num dist3:', sample_num)
        return dataset3
    ...
    def generate_train_test(self):
        ...
        print('sum num_samples of train data:',sum(test_data['num_samples']))
...
def genrate_cifar10():
    ...
    generator=DataDivider(data_list,label_list,num_users=100,a=50,division_ratio=[1/3, 1/3,1/3],train_test_ratio=0.5,savepath='./cifar10',num_class=10)
    generator.save_data()
