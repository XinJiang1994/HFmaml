...
for user in range(100):
    for j in range(2):
        ...
        y[user] += mnist_label[l][idx[l]:idx[l]+5].tolist()
        idx[l] += 5
        #print(y[user])
print(idx)
...
props = np.array([[[len(v)-4500]] for v in mnist_data])*props/np.sum(props,(1,2), keepdims=True)#change 100 to 5000
...
for user in trange(100):
    for j in range(2):
        ...
        if idx[l] + num_samples < len(mnist_data[l]):
            ...
            y[user] += mnist_label[l][idx[l]:idx[l]+num_samples].tolist()
            idx[l] += num_samples
