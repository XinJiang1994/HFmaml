...
class Model(BaseModel):
    ...
    def get_input(self):
        ...
        labels_train = tf.placeholder(tf.float32, shape=[None, 100], name='labels_train')
        features_test = tf.placeholder(tf.float32, shape=[None, 32,32,3], name='features_test')
        labels_test = tf.placeholder(tf.float32, shape=[None,100], name='labels_test')
    ...
    def forward_func(self,inp, weights, w_names , reuse = False):
        ...
        h_pool4 = tf.nn.avg_pool(h_conv4, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME',
                              name='avg_pool1')
        # print(h_pool4)
        h_pool_shape=h_pool4.get_shape().as_list()
        h = h_pool_shape[1]
        w = h_pool_shape[2]
        c = h_pool_shape[3]
        flatten= tf.reshape(h_pool4, [-1, h * w * c])
    ...
    def construct_weights(self):
        ...
        W_fc1 = weight_variable([1024,100], name='W_fc1')
