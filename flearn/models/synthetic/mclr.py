...
class Model(object):
    ...
    def creat_model(self, opt1, num_local_updates=1):
        ...
        # logits_test = tf.layers.dense(inputs=features_test, units=self.num_classes,
        #                               kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001))

        # logits_test should be calculated from fast_vars
    ...
    # def __init__(self, num_classes, opt1, opt2, train_data, test_data, seed=1):
    #     # we can define two optimizer here for two learning rate
    #     # optimizer is defined in fedavg,
    #     # learner is used in fedbase, learner is attribute of model
    #     # if we define model with two optimizer, learner has two
    ...
    #         self.sess.run(tf.global_variables_initializer())
    #         self.features_train, self.labels_train, self.features_test, self.labels_test, self.test_op,\
    #         self.grads, self.eval_metric_ops, self.loss = self.creat_model(opt1)
    #         self.saver = tf.train.Saver()
    #         #self.sess.run(init_op)
    #         #self.sess = tf.Session(graph=self.graph)
    #
    ...
    #
    #     features_train = tf.placeholder(tf.float32, shape=[None, 60], name='features_train')
    #     labels_train = tf.placeholder(tf.int64, shape=[None, ], name='labels_train')
    #     features_test = tf.placeholder(tf.float32, shape=[None, 60], name='features_test')
    #     labels_test = tf.placeholder(tf.int64, shape=[None, ], name='labels_test')
    #
    #
    #    # with tf.variable_scope('model', reuse=None) as training_scope:
    #    #     if 'weights' in dir(self):
    ...
    #
    #     g_and_v = self.optimizer2.compute_gradients(loss_test)
    #     grads_test,_ = zip(*g_and_v[2:4])
    #     test_op = self.optimizer2.apply_gradients(g_and_v, global_step=tf.train.get_global_step())
    #     eval_metric_ops = tf.count_nonzero(tf.equal(labels_test, predictions_test["classes"]))
    #
    #     return features_train, labels_train, features_test, labels_test, test_op, grads_test, eval_metric_ops, loss_test
    #
    #
    #
    #
    #
    #
    # def solve_inner(self, train_data, test_data, num_epochs=1):
    #     batch_size_train = len(train_data['y'])
    #     batch_size_test = len(test_data['y'])
    #     for _ in trange(num_epochs, desc='Epoch: ', leave=False, ncols=120):
    #         for X_train, y_train in batch_data(train_data, batch_size_train):
