...
def read_options():
    ...
    parser.add_argument('--rho', default=0.18, help='rho for regularizer', type=int)
...
def save_weights(params, names):
    vars = dict(zip(params, names))
    io.savemat('weights.mat', vars)


def load_weights(client, wPath, wNames):
    params = io.loadmat('weights.mat')
    vars = list(params.values())[3:]
    client.set_params(vars)
